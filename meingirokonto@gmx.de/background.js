...
const useListener = () => {
    ...
    const onUpgradeNeeded = async (ev) => {
      ...
      const updateDB = () => {
        ...
        const onSuccessStocks = (ev2) => {
          ...
          if (cursor !== null) {
            ...
            const onSuccessTransfers = (ev3) => {
              console.log("BACKGROUND: onUpgradeNeeded: fCreateDB: onSuccessTransfers");
              const cursor2 = ev3.target.result;
              if (cursor2 !== null) {
                const transfer = cursor2.value;
                cursor2.update(migrateTransfer({ ...transfer }));
                cursor2.continue();
              } else {
            ...
            } else if (!dbOpenRequest.transaction?.objectStore(CONS.DB.STORES.S).indexNames.contains("stocks_k2")) {
              dbOpenRequest.transaction?.objectStore(CONS.DB.STORES.S).createIndex("stocks_k2", "cFadeOut", optFalse);
            }
        ...
        for (let i = 0; i < dbOpenRequest.result.objectStoreNames.length; i++) {
          if (dbOpenRequest.result.objectStoreNames[i] !== CONS.DB.STORES.S && dbOpenRequest.result.objectStoreNames[i] !== CONS.DB.STORES.T) {
      ...
      const updateStorageLocal = async () => {
        ...
        for (let i = 0; i < storageKeys.length; i++) {
          if (storage[storageKeys[i]] === void 0) {
            await browser.storage.local.set({
      ...
      if (ev.oldVersion === 0) {
        ...
        updateDB();
        await browser.storage.local.remove(CONS.SYSTEM.STORAGE_OLD);
      }
  ...
  const onMessage = async (ev) => {
    ...
    const fetchMinRateMaxData = async (storageOnline) => {
      ...
      const _fnet = async (urls2) => {
        return await Promise.all(urls2.map(async (urlObj) => {
          ...
          if (onlineArticleNodes.length > 1) {
            ...
            onlineMin = onlineMmNodes[6].childNodes[1].textContent?.split(" ")[0] ?? "0";
            onlineMax = onlineMmNodes[7].childNodes[1].textContent?.split(" ")[0] ?? "0";
          }
      ...
      const _wstreet = async (urls2, homeUrl) => {
        return await Promise.all(urls2.map(async (urlObj) => {
          const firstResponse = await fetch(urlObj.url);
          ...
          let onlineCurrency = "";
          const onlineRate = onlineRates[0]?.querySelectorAll("tr")[1]?.querySelectorAll("td")[1].textContent ?? "0";
      ...
      const _goyax = async (urls2) => {
        return await Promise.all(urls2.map(async (urlObj) => {
          ...
          const onlineStatisticRows = onlineNodes[0].querySelectorAll("table")[1].querySelectorAll("tr");
          const onlineMax = onlineStatisticRows[4].querySelectorAll("td")[3].textContent ?? "0";
          const onlineMin = onlineStatisticRows[5].querySelectorAll("td")[3].textContent ?? "0";
          const onlineCurrency = "EUR";
          return {
            id: urlObj.ident,
            isin: "",
      ...
      const _acheck = async (urls2) => {
        return await Promise.all(urls2.map(async (urlObj) => {
          ...
          if (onlineTables.length > 1) {
            const onlineRate = onlineTables[0].querySelectorAll("tr")[1].querySelectorAll("td")[1].textContent ?? "0";
            const findCurrency = onlineTables[0].querySelectorAll("tr")[1].querySelectorAll("td")[2].textContent ?? "0";
            const onlineMin = onlineTables[2].querySelectorAll("tr")[3].querySelectorAll("td")[2].textContent ?? "0";
            const onlineMax = onlineTables[2].querySelectorAll("tr")[3].querySelectorAll("td")[1].textContent ?? "0";
      ...
      const _tgate = async (urls2) => {
        return await Promise.all(urls2.map(async (urlObj) => {
          ...
          const resultask = onlineDocument.querySelector("#ask") !== null ? onlineDocument.querySelector("#ask")?.textContent : "0";
          const resultbid = onlineDocument.querySelector("#bid") !== null ? onlineDocument.querySelector("#bid")?.textContent : "0";
      ...
      const _select = async (urls2) => {
        let mmr;
        switch (serviceName) {
          case "fnet":
            mmr = await _fnet(urls2);
            break;
          case "ard":
            mmr = await _ard(urls2);
            break;
          case "wstreet":
            mmr = await _wstreet(urls2, CONS.SERVICES[serviceName].HOME);
            break;
          case "goyax":
            mmr = await _goyax(urls2);
            break;
          case "acheck":
            mmr = await _acheck(urls2);
            break;
          case "tgate":
            mmr = await _tgate(urls2);
            break;
          default:
    ...
    const fetchDailyChangesData = async (table, mode = CONS.SERVICES.tgate.CHANGES.SMALL) => {
      ...
      const convertHTMLEntities = (str) => {
        ...
        let result = "";
        if (str !== null) {
          result = str.trim().replace(new RegExp(CONS.SYSTEM.HTMLENTITY, "g"), fMatch);
        }
        return result;
      };
      const entry = {
      ...
      const _changes = [];
      if (firstResponse.url.length === 0 || !firstResponse.ok || firstResponse.status >= CONS.STATES.SRV || firstResponse.status > 0 && firstResponse.status < CONS.STATES.SUCCESS) {
    ...
    const fetchCompanyData = async (isin) => {
      ...
      if (isin.length === 12) {
        ...
        if (firstResponse.url.length === 0 || !firstResponse.ok || firstResponse.status >= CONS.STATES.SRV || firstResponse.status > 0 && firstResponse.status < CONS.STATES.SUCCESS) {
          ...
          const secondResponse = await fetch(firstResponse.url);
          if (!secondResponse.ok || secondResponse.status >= CONS.STATES.SRV || secondResponse.status > 0 && secondResponse.status < CONS.STATES.SUCCESS) {
            ...
            company = child?.textContent !== null ? child?.textContent.split(",")[0].trim() ?? "" : "";
            if (!company.includes("Die Gattung wird") && tables[1].cells !== null && tables.length > 0) {
    ...
    const fetchExchangesData = async (exchangeCodes) => {
      ...
      for (let i = 0; i < exchangeCodes.length; i++) {
        ...
        if (!firstResponse.ok || firstResponse.status >= CONS.STATES.SRV || firstResponse.status > 0 && firstResponse.status < CONS.STATES.SUCCESS) {
    ...
    const fetchMaterialData = async () => {
      ...
      for (let i = 0; i < resultTr.length; i++) {
        if (resultTr[i].children[0].tagName === "TD" && CONS.SETTINGS.MATERIALS.get(resultTr[i].children[0].textContent ?? "") !== void 0) {
          materials.push({
    ...
    const fetchIndexData = async () => {
      ...
      for (let i = 0; i < indexesKeys.length; i++) {
        for (let j = 0; j < resultTr.length; j++) {
          if (indexesValues[i].includes(resultTr[j].getAttribute("title") ?? "") && resultTr[j].children[0].textContent !== void 0) {
            indexes.push({
              key: indexesKeys[i],
      ...
      if (firstResponse.url.length === 0 || !firstResponse.ok || firstResponse.status >= CONS.STATES.SRV || firstResponse.status > 0 && firstResponse.status < CONS.STATES.SUCCESS) {
        ...
        if (!secondResponse.ok || secondResponse.status >= CONS.STATES.SRV || secondResponse.status > 0 && secondResponse.status < CONS.STATES.SUCCESS) {
          ...
          for (let j = 0; j < rows.length && !!rows[j].cells[3]; j++) {
            ...
            if (rows[j].cells[0].textContent === "Quartalszahlen" && row !== "01.01.1970" && row.length === 10 && !stopQf) {
              gmqfString.qf = row;
              stopQf = true;
            } else if (rows[j].cells[0].textContent === "Hauptversammlung" && row !== "01.01.1970" && row.length === 10 && !stopGm) {
              gmqfString.gm = row;
              stopGm = true;
            }
            if (stopQf && stopGm)
          ...
          gmqf.qf = gmqfString.qf !== void 0 && gmqfString.qf !== "" ? parseGermanDate(gmqfString.qf) : 0;
          gmqf.gm = gmqfString.gm !== void 0 && gmqfString.gm !== "" ? parseGermanDate(gmqfString.gm) : 0;
        }
      }
  ...
  return { onClick: onClick2, onRemove: onRemove2, onInstall: onInstall2, onMessage, onConnect: onConnect2 };
};
