  ...
  return {
    ...
    getUI: () => {
      ...
      if (uiLang.includes("-")) {
        ...
        code = CONS.CURRENCIES.CODE.get(uiLang.split("-")[1]) ?? CONS.DEFAULTS.CURRENCY;
      ...
      result.fontSize = window.getComputedStyle(document.body, null).getPropertyValue("font-size");
      return result;
    },
    group: (count, size = 2) => {
      const ar = [];
    ...
    toNumber: (str) => {
      ...
      if (str !== null && str !== void 0) {
        ...
        const b = a.split(",");
        if (b.length === 2) {
          const tmp2 = a.trim().replace(/\s|\.|\t|%/g, "").replace(",", ".");
          result = Number.isNaN(Number.parseFloat(tmp2)) ? 0 : Number.parseFloat(tmp2);
        } else if (b.length > 2) {
          let tmp = "";
